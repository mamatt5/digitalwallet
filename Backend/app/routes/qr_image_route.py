...
async def upload_image_route(
    ...
    qr_image_service: QRImageService = Depends(QRImageService)
) -> Dict:
    try:
        file_data = await file.read()
        image = qr_image_service.create_or_update_image(name=name, data=file_data, merchant_id=merchant_id)
        return {"id": image.qrimage_id, "name": image.name, "merchant_id": image.merchant_id}
    except Exception as e:
        raise HTTPException(status_code=400, detail="Image upload failed") from e
...
@router.put("/update")
async def update_qr_image_route(
    name: str, 
    merchant_id: int, 
    file: UploadFile = File(...),  
    qr_image_service: QRImageService = Depends(QRImageService)
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Image update failed") from e
